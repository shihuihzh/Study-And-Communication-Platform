    ...
    public static Query generateQuery(String searchTerm, Class searchedEntity, Session sess, Analyzer defaultAnalyzer) throws ParseException {
        ...
        if (searchTerm.equals("*")) {
            ...
            try {
                ...
                Collection<String> fnames = new ArrayList<String>();
                for(FieldInfo field : fieldInfos) {
                	if(!"_hibernate_class".equals(field.name)) {
                		fnames.add(field.name);                		
                	}
                }
                
                // To search on all fields, search the term in all fields
                String[] queries = new String[fnames.size()];
                for (int i = 0; i < queries.length; ++i) {
                    queries[i] = searchTerm;
                }

                qry = MultiFieldQueryParser.parse(Version.LUCENE_35, queries, (String[])fnames.toArray(), analyzer);
            } finally {
